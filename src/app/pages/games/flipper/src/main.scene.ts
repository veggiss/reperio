...
export class MainScene extends Phaser.Scene {
  ...
  create () {
    ...
    for (let posY = 1; posY <= 2; posY++) {
      for (let posX = 1; posX <= 2; posX++) {
        let cardUpper = new ImageCard({scene: this, parent: this.imageCardBg, upper: true, posX: posX, posY: posY, image: 'image_' + i, answer: this.roundData[i].answer, pos: i});
  ...
  flipImageCards(open) {
    ...
    console.log("FLIP IMAGE CARD", open, this.state);
